...
<head>
    <meta charset="UTF-8">
    <title> Incident Report | Review Pending Report </title>
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    
    <link rel="stylesheet" href="ir-report-pending.css">
...
$sql = "SELECT * from pending_report ORDER BY date";
$result=mysqli_query($con,$sql);             
    while ($rec=mysqli_fetch_array($result,MYSQLI_ASSOC))
 {

  $rno=$rec['report_no'];
  $email=$rec['reported_by'];
  $a=$rec['date'];
  $b=$rec['time'];
  $c=$rec['loc'];
  ...
  $pfp=$rec['image'];
    ...
    <td>$d</td>
    <td>$e</td>
    <td> <button class='review'><a href=ir-pending-form.php?report_no=$rno&reported_by=$email>Review</a></button> </td>
    </tr>";
